...
def _explode_on_compute(shape, chunks):
    ...
    return base.map_blocks(_boom, dtype=base.dtype, meta=np.empty((0,), base.dtype))
...
def _column_ds(thickness_m=10.0, nlayers=5, ny=3, nx=4):
    ...
    thknss = xr.DataArray(
        thk_pa, dims=("k", "y", "x"), coords={"k": np.arange(1, nlayers + 1)}
    )
...
def test_layer_centre_depth_strictly_increasing_with_massless_layers():
    ...
    thknss = xr.DataArray(
        thk[:, None, None], dims=("k", "y", "x"), coords={"k": np.arange(1, 6)}
    )
...
def test_regrid_vertical_conservative_builds_lazily():
    ...
    out = xhycom.regrid_vertical(ds, depth=[5.0, 15.0, 25.0], method="conservative")
...
def test_regrid_vertical_masks_below_bottom():
    ...
    out = xhycom.regrid_vertical(ds, depth=[100.0], method="linear", mask_edges=True)
...
def _curvilinear_ds(nlayers=3):
    ...
    ny, nx = lat2d.shape
    thknss = xr.DataArray(
        np.full((nlayers, ny, nx), 10.0 * _ONEM),
        dims=("k", "y", "x"),
        coords={"k": np.arange(1, nlayers + 1)},
    )
    temp = xr.DataArray(
        np.broadcast_to(lat2d, (nlayers, ny, nx)).copy(),
        dims=("k", "y", "x"),
        coords={"k": np.arange(1, nlayers + 1)},
    )
...
def test_regrid_horizontal_recovers_field():
    ...
    out = xhycom.regrid_horizontal(ds, lon=tgt_lon, lat=tgt_lat, method="bilinear")
...
def test_weights_cache_reused_and_consistent(tmp_path):
    ...
    lat = np.linspace(42, 48, 7)
    wfile = tmp_path / "w.nc"
    first = xhycom.regrid_horizontal(
        ds, lon=lon, lat=lat, method="bilinear", weights=wfile
    )
    assert wfile.exists()
    second = xhycom.regrid_horizontal(
        ds, lon=lon, lat=lat, method="bilinear", weights=wfile
    )  # reuse path
...
def test_nan_pole_blanks_lat_90_row_by_default(tp0):
    ...
    out = xhycom.regrid_horizontal(
        ds, lon=lon, lat=lat, grid=tp0.grid, method="bilinear"
    )
    ...
    raw = xhycom.regrid_horizontal(
        ds, lon=lon, lat=lat, grid=tp0.grid, method="bilinear", nan_pole=False
    )
...
def test_regrid_vertical_conservative_preserves_column_integral():
    ...
    out = xhycom.regrid_vertical(
        ds, depth=[12.5, 37.5], method="conservative", mask_edges=False
    )
...
def test_regrid_vertical_conservative_constant():
    ...
    out = xhycom.regrid_vertical(
        ds, depth=[10.0, 30.0], method="conservative", mask_edges=False
    )
...
def test_regrid_horizontal_conservative_preserves_constant():
    ...
    ny, nx = lat2d.shape
    temp = xr.DataArray(
        np.full((1, ny, nx), 5.0), dims=("k", "y", "x"), coords={"k": [1]}
    )
    ...
    out = xhycom.regrid_horizontal(
        ds,
        lon=np.linspace(2, 8, 9),
        lat=np.linspace(42, 48, 9),
        grid=grid,
        method="conservative",
    )
...
def test_regrid_horizontal_conservative_is_thickness_weighted():
    ...
    temp = xr.DataArray(
        np.full((1, ny, nx), 3.0), dims=("k", "y", "x"), coords={"k": [1]}
    )
...
def test_regrid_horizontal_accepts_depth_levels():
    ...
    ds = xr.Dataset(
        ...
        lon=(("y", "x"), lon2d), lat=(("y", "x"), lat2d), depth=("depth", [5.0, 50.0])
    )
    out = xhycom.regrid_horizontal(
        ds,
        lon=np.linspace(2, 8, 9),
...
def test_regrid_to_hycom_conservative_preserves_constant():
    ...
    out = xhycom.regrid_to_hycom(
        _regular_product(7.0), grid=grid, method="conservative"
    )
    v = out["temp"].values
...
def test_regrid_to_hycom_applies_like_mask():
    ...
    out = xhycom.regrid_to_hycom(
        _regular_product(3.0), grid=grid, method="bilinear", like=like
    )
